#!/bin/bash
# fastq2tracks v3.0.4 — Blacklist filtering batch
...
LOG_DIR="${OUT_DIR}/blacklist_logs_${TIMESTAMP}"
mkdir -p "$LOG_DIR"
...
tail -n +2 "$SAMPLESHEET" | while IFS=',' read -r sample_id fastq_1 fastq_2 layout genome \
    assay factor condition treatment cell_type replicate tech_rep \
    is_control control_id macs2_mode blacklist rest; do
    ...
    IN_BAM="${IN_DIR}/${sample_id}_bioR${replicate}_dedup.bam"
    [[ ! -f "$IN_BAM" ]] && { log "SKIP $sample_id — dedup BAM not found: $IN_BAM"; continue; }

    OUT_BAM="${OUT_DIR}/${sample_id}_bioR${replicate}_dedup_blFilt.bam"
    [[ -f "$OUT_BAM" ]] && { log "SKIP $sample_id (filtered BAM exists)"; continue; }

    wait_for_slot
    (bash "$(dirname "$0")/blacklist_filter.sh" "$IN_BAM" "$blacklist" "$OUT_DIR" \
        > "$LOG_DIR/${sample_id}.log" 2>&1) &
    pids+=($!)
    log "STARTED: $sample_id"
done
