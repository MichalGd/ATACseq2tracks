...
usage() {
    echo "Usage: $0 <index> <input_folder> <output_folder> [max_jobs]"
    echo ""
    echo "Parameters:"
    echo "  index          - Path to Bowtie2 index (e.g., /path/to/hg38)"
    echo "  input_folder   - Directory containing trimmed FASTQ files (*_val_1.fq.gz)"
    echo "  output_folder  - Directory where aligned BAM files will be saved"
    echo "  max_jobs       - Maximum number of parallel jobs (default: 8, optional)"
    echo ""
    echo "Examples:"
    echo "  $0 /path/to/hg38 /dysk2/fastqs /dysk2/bams 8"
    echo "  $0 /path/to/hg38 /dysk2/fastqs /dysk2/bams     # Uses default 8 jobs"
    echo ""
...
inFolder="$2"
outFolder="$3"
max_jobs="${4:-8}"  # Default to 8 if not provided
...
# Validate that the Bowtie2 script exists and is executable
BOWTIE2_SCRIPT="/home/micgdu/workflows/RNAseq/scripts/bowtie2_dovetail_pairedEnd_MMusculus.2.1.sh"

if [[ ! -f "$BOWTIE2_SCRIPT" ]]; then
    echo "ERROR: Bowtie2 script not found: $BOWTIE2_SCRIPT"
    exit 1
fi

if [[ ! -x "$BOWTIE2_SCRIPT" ]]; then
    echo "ERROR: Bowtie2 script is not executable: $BOWTIE2_SCRIPT"
    echo "Please run: chmod +x $BOWTIE2_SCRIPT"
    exit 1
fi

# Validate index parameter
if [[ ! -d "$(dirname "$index")" ]]; then
    echo "ERROR: Index directory does not exist: $(dirname "$index")"
    exit 1
fi

# Set up logging in output folder
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
LOG_SUBFOLDER="$outFolder/bowtie2_batch_logs_${TIMESTAMP}"
MAIN_LOG="$LOG_SUBFOLDER/bowtie2_batch_${TIMESTAMP}.log"
ERROR_LOG="$LOG_SUBFOLDER/bowtie2_batch_errors_${TIMESTAMP}.log"
JOBS_LOG_DIR="$LOG_SUBFOLDER/individual_jobs"
MASTER_PID_FILE="$LOG_SUBFOLDER/bowtie2_batch_${TIMESTAMP}.pid"
...
log_main "=== Starting Bowtie2 Batch Processing (Continuous Job Replacement) ==="
log_main "Index: $index"
...
log_main "Bowtie2 script: $BOWTIE2_SCRIPT"
log_main "Log folder: $LOG_SUBFOLDER"

# Find trimmed FASTQ files that need processing
all_samples=($(ls -f *_val_1.fq.gz 2>/dev/null))
samples_to_process=()

log_main "Scanning ${#all_samples[@]} trimmed FASTQ files..."
...
for sample in "${all_samples[@]}"; do
    ...
    sample_name=$(basename "$sample" _val_1.fq.gz)
    if [[ -f "$outFolder/${sample_name}.sorted_stChr.bam" && -f "$outFolder/${sample_name}.sorted_stChr.bai" ]] && samtools quickcheck "$outFolder/${sample_name}.sorted_stChr.bam" 2>/dev/null; then
        log_main "SKIP: $sample (aligned BAM exists)"
        continue
    fi
...
start_job() {
    local sample="$1"
    local sample_name=$(basename "$sample" _val_1.fq.gz)
    ...
    (
        ...
        # Initialize log file
        cat > "$job_log" << EOF
=== Bowtie2 Alignment Job ===
Sample: $sample
Started: $start_time
PID: $$
Index: $index
Input R1 file: $inFolder/$sample
Input R2 file: $inFolder/${sample/_val_1.fq.gz/_val_2.fq.gz}
Output BAM: $outFolder/${sample_name}.sorted_stChr.bam
Bowtie2 script: $BOWTIE2_SCRIPT
Working directory: $(pwd)
=================================
EOF
        
        # Execute Bowtie2 script
        echo "Executing: $BOWTIE2_SCRIPT \"$index\" \"$inFolder/$sample\" \"$outFolder\"" >> "$job_log"
        echo "==================== BOWTIE2 OUTPUT ====================" >> "$job_log"
        if "$BOWTIE2_SCRIPT" "$index" "$inFolder/$sample" "$outFolder" >> "$job_log" 2>> "$job_error_log"; then
            end_time=$(date)
            echo "==================== JOB COMPLETED ====================" >> "$job_log"
            echo "SUCCESS: $end_time" >> "$job_log"
            # Verify output files were created
            if [[ -f "$outFolder/${sample_name}.sorted_stChr.bam" && -f "$outFolder/${sample_name}.sorted_stChr.bai" ]]; then
                echo "Output files verified: $outFolder/${sample_name}.sorted_stChr.bam and .bai" >> "$job_log"
                exit 0
            else
                echo "ERROR: Bowtie2 output files not created despite success exit code" >> "$job_log"
...
for sample in "${samples_to_process[@]}"; do
    sample_name=$(basename "$sample" _val_1.fq.gz)
    output_file="$outFolder/${sample_name}.sorted_stChr.bam"
...
log_main "=== Bowtie2 batch processing completed ==="
