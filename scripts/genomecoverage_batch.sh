#!/usr/bin/env bash
# ATACseq2tracks v4.0.0 - strict fragment/read CPM bigWig batch
set -euo pipefail
...
wait_one() {
    ...
    if ! wait "$pid"; then log "ERROR: CPM bigWig failed: $label"; failures=$((failures + 1)); fi
...
while IFS=',' read -r sample_id fq1 fq2 layout genome assay factor condition treatment cell_type replicate tech_rep is_control rest; do
    ...
    [[ -s "${OUT_DIR}/${key}_dedup_blFilt_CPM.bw" ]] && { log "SKIP: $key"; continue; }
...
(( failures == 0 )) || { log "FATAL: ${failures} CPM bigWig job(s) failed"; exit 1; }
log "CPM bigWig batch complete"
